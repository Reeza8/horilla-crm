...
from horilla.contrib.core.models import FieldRequirement, HorillaContentType
...
class FieldRequirementForm(HorillaModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        configurable_models = get_configurable_models()
        self.fields["content_type"].queryset = HorillaContentType.objects.filter(
            pk__in=[
                HorillaContentType.objects.get_for_model(model).pk
        ...
        self.fields["content_type"].widget.attrs.update(
            {
                "hx-get": reverse_lazy("core:field_requirement_field_choices"),
    ...
    def _resolve_field_choices(self):
        ...
        if model is None:
            return []
        return get_field_choices(model)
