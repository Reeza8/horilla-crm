"""
Compose HorillaTabView subclasses with extension mixins (_inherit_tab).
"""
...
from horilla.extension.tab.registry import TabExtensionSpec, get_tab_extensions_for
...
def _spec_to_mixin(spec: TabExtensionSpec) -> type:
    """
    ...
    horilla.extension._super_rebind), so ``super().get_tabs(self)`` correctly
    chains to the next extension or the target view — calling the target
    base explicitly (e.g. ``CompanyInformationTabView.get_tabs(self)``) is no
    longer required, though it still works for a single registered extension.
    """
...
def compose_tab_view_class(target_path: str, target: type | None = None) -> type:
    """
    Compose target tab view with registered extensions.

    MRO: Composed -> ExtN -> ... -> Ext1 -> Target
    """
    if getattr(target, "__horilla_tab_composed__", False):
        return target

    target = target or _import_tab_view_class(target_path)
    specs = get_tab_extensions_for(target_path)
    if not specs:
        return target
    ...
    composed.__horilla_tab_composed__ = True
    composed.__horilla_tab_path__ = target_path
    composed.__wrapped_tab_view__ = target
